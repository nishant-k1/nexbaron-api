...
export async function createCheckout(req: Request, res: Response) {
  try {
    ...
    const { Order, Lead, InvoiceCounter, User } = getDivisionModels(
      req.division!,
    );
  ...
  try {
    ...
    res.json({
      ...
      orders: orders.map((order) => {
        ...
        const plan = order.service
          ? servicePricingPlans[order.service]
          : undefined;
...
export async function downloadReceipt(req: Request, res: Response) {
  try {
    ...
    if (
      !req.userId ||
      !Types.ObjectId.isValid(req.userId) ||
      !orderId ||
    ...
    const invoice = escapeHtml(
      order.invoiceNumber || order._id.toString().slice(-8).toUpperCase(),
    );
    const brand =
      req.division === "digital" ? "Nexbaron Digital" : "Nexbaron Print";
...
<div class="footer">${brand} · nexbaron.com · This is a computer-generated receipt.</div>
    ...
    res.setHeader(
      "Content-Disposition",
      `inline; filename="receipt-${invoice}.html"`,
    );
    res.send(html);
  } catch (error) {
...
export async function verifyPayment(req: Request, res: Response) {
  try {
    ...
    if (
      ...
      res
        .status(400)
        .json({ success: false, message: "Missing payment references" });
      return;
    }
    ...
    if (!razorpayConfigured() && process.env.NODE_ENV === "production") {
      res
        .status(503)
        .json({ success: false, message: "Payments are not configured" });
      return;
    }
    if (
      razorpayConfigured() &&
      !verifyPaymentSignature(body as Required<typeof body>)
    ) {
      res
        .status(400)
