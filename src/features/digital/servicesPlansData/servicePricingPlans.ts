...
type DigitalMarketingCategory = keyof typeof nexbaronPublicDigitalMarketingServices
...
// Each plan's price is its OWN tier price — `includes` means a higher tier
// bundles every tier below it (Launch ⊂ Growth ⊂ Scale), so the full price of
// a plan is the sum of its chain. These own-tier figures match the retired
// itemized catalog totals.
...
function engineeringService<C extends EngineeringCategory>(
  ...
  return { domain: 'engineering', category, service, scope } as PublicPlanServiceRef
}
...
function digitalMarketingService<C extends DigitalMarketingCategory>(
  ...
  return { domain: 'digitalMarketing', category, service, scope } as PublicPlanServiceRef
}
...
const servicePricingPlans: Record<string, PricingPlan> = {
  launch: {
    ...
    price: { oneTime: 13580, monthly: 1154, annual: 12420 },
    services: [
      engineeringService('website', 'businessWebsite', { pages: 5 }),
      engineeringService('integrations', 'whatsappIntegration', { type: 'contact' }),
      engineeringService('analytics', 'webAnalytics', { type: 'basic' }),
      digitalMarketingService('seo', 'googleBusinessProfile', { type: 'setup' }),
    ],
  },
  ...
  growth: {
    ...
    featured: true,
    includes: ['launch'],
    price: { oneTime: 1700, monthly: 11550, annual: 115500 },
    services: [
      digitalMarketingService('seo', 'seo', { type: 'localBusiness' }),
      digitalMarketingService('socialMedia', 'socialMediaManagement', { postsPerMonth: 8 }),
      digitalMarketingService('contentMarketing', 'socialMediaCreatives', { creativesPerMonth: 8 }),
      digitalMarketingService('leadGeneration', 'leadGeneration', { campaignsPerMonth: 1 }),
      digitalMarketingService('analytics', 'conversionTracking', { type: 'standard' }),
      digitalMarketingService('analytics', 'campaignReporting', { frequency: 'monthly' }),
    ],
  },
  ...
  scale: {
    ...
    timelineMode: 'phased',
    foundationDays: 30,
    price: { oneTime: 0, monthly: 19035, annual: 190350 },
    services: [
      digitalMarketingService('paidAdvertising', 'googleAds', { campaigns: 2, remarketing: true }),
      digitalMarketingService('paidAdvertising', 'metaAds', { campaigns: 2, remarketing: true }),
      digitalMarketingService('contentMarketing', 'shortFormVideo', { videosPerMonth: 4 }),
      digitalMarketingService('analytics', 'marketingAnalytics', { frequency: 'monthly' }),
    ],
  },
