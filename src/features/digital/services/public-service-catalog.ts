...
const NEED_SECTIONS: (PublicServiceSection & { outcome: string; services: PublicServiceRef[] })[] = [
  {
    id: 'build-online-presence',
    slug: 'build-online-presence',
    title: 'Build My Online Presence',
    subtitle: 'Websites, landing pages, ecommerce, Google presence, and analytics to look credible online.',
  ...
  },
  {
    ...
    subtitle: 'SEO, ads, local marketing, landing page campaigns, and tracking to turn attention into enquiries.',
    ...
    services: [
      ...
      'digitalMarketing.leadGeneration.leadGeneration',
      'digitalMarketing.leadGeneration.landingPageCampaigns',
      'digitalMarketing.analytics.marketingAnalytics',
      'digitalMarketing.analytics.campaignReporting',
      'digitalMarketing.analytics.conversionTracking',
    ],
  },
  {
    ...
    subtitle: 'Social media, blogs, creatives, short videos, copywriting, influencer campaigns, and email marketing.',
    ...
    services: [
      ...
      'digitalMarketing.contentMarketing.contentMarketing',
      'digitalMarketing.contentMarketing.blogContent',
      'digitalMarketing.contentMarketing.socialMediaCreatives',
      'digitalMarketing.contentMarketing.shortFormVideo',
      'digitalMarketing.contentMarketing.copywriting',
      'digitalMarketing.influencerMarketing.influencerMarketing',
      'digitalMarketing.influencerMarketing.creatorCampaignManagement',
  ...
  },
  {
    ...
    subtitle: 'WhatsApp, booking, email, CRM, workflow, document, process, and AI automation to save time.',
  ...
  },
  {
    ...
    subtitle: 'CRMs, portals, dashboards, reporting systems, booking systems, and internal business tools.',
  ...
  },
  {
    ...
    subtitle: 'Integrations for WhatsApp, email, SMS/OTP, CRM, payments, calendar, accounting, shipping, and APIs.',
...
function getServiceLabel(ref: PublicServiceRef): string {
  const [domain, category, service] = ref.split('.') as [PublicServiceDomain, string, string]
...
function getAllSourceRefs(): PublicServiceRef[] {
  return Object.entries(PUBLIC_SERVICE_SOURCES).flatMap(([domain, categories]) =>
    Object.entries(categories).flatMap(([category, services]) =>
      Object.keys(services).map((service) => `${domain}.${category}.${service}` as PublicServiceRef),
    ),
  )
}

function assertEveryPublicServiceIsGrouped() {
  const groupedRefs = new Set(NEED_SECTIONS.flatMap((section) => section.services))
  ...
  if (missing.length || unknown.length) {
    throw new Error(`Public service grouping mismatch. Missing: ${missing.join(', ') || 'none'}. Unknown: ${unknown.join(', ') || 'none'}`)
  }
}
...
export function getCanonicalPublicServiceSections(): PublicServiceSection[] {
  return NEED_SECTIONS.map(({ services: _services, outcome: _outcome, ...section }) => section)
}
