...
export async function submitQuote(req: Request, res: Response) {
  try {
    ...
    const user =
      req.userId && isObjectId(req.userId)
        ? await User.findById(req.userId).then((found) =>
            found && found.division === division ? found : null,
          )
        : null;
    ...
    if (clientRequestId && !/^[A-Za-z0-9_-]{16,100}$/.test(clientRequestId)) {
      res
        .status(400)
        .json({ success: false, message: "Invalid quote request identifier" });
      return;
    }
    if (clientRequestId) {
      const existing = user
        ...
        : await Quote.findOne({
            "customer.email": String(body.email || "")
              .trim()
              .toLowerCase(),
            clientRequestId,
          });
    ...
    if (!name || name.length < 2) {
      res
        .status(400)
        .json({ success: false, message: "Please provide a valid name" });
      return;
    }
    ...
    if (division === "print") {
      ...
      const items: { product: string; quantity: number }[] = Array.isArray(
        body.items,
      )
        ? body.items
            .map((it: any) => ({
            ...
            .filter(
              (it: any) =>
                it.product &&
                it.quantity >= 1 &&
                it.quantity <= MAX_PRINT_QUANTITY,
            )
        : [];
      ...
      details.deliveryPincode =
        String(body.deliveryPincode || "").trim() || undefined;
      ...
      details.businessType =
        String(body.businessType || "").trim() || undefined;
    ...
    const lead = await Lead.findOneAndUpdate(
      { division, email, phone: phone || "" },
      {
        $set: {
          ...
          company:
            division === "print"
              ? (details.company as string | undefined)
              : undefined,
          subject:
            division === "digital"
              ? (selection.planIds as string[]).join(", ")
              : selection.product,
          message: String(details.notes || body.message || "").trim(),
          requirement: division === "print" ? selection.product : undefined,
          quantity:
            division === "print" ? String(selection.quantity) : undefined,
        },
...
export async function myQuotes(req: Request, res: Response) {
  try {
    ...
    if (!isObjectId(req.userId)) {
      res
        .status(401)
        .json({ success: false, message: "Invalid authentication subject" });
      return;
    }
    ...
    if (typeof body.response === "object" && body.response !== null) {
      ...
      quote.response = {
        ...
        ...(typeof response.price === "number"
          ...
          : {}),
        ...(typeof response.monthlyPrice === "number"
          ? { monthlyPrice: response.monthlyPrice }
          : {}),
        ...(typeof response.validityDays === "number"
        ...
        ...(Array.isArray(response.channels)
          ? { channels: response.channels.map(String) }
          : {}),
      };
    }
...
export async function sendQuote(req: Request, res: Response) {
  try {
    ...
    if (
      ...
      res
        .status(400)
        .json({ success: false, message: "Please set a quote price" });
      return;
    }
    if (
    ...
    quote.response = {
      price: body.price,
      monthlyPrice:
        typeof body.monthlyPrice === "number" ? body.monthlyPrice : undefined,
      validityDays:
        typeof body.validityDays === "number" ? body.validityDays : 7,
    ...
    if (!emailed) {
      ...
      res.status(502).json({
        success: false,
        message:
          "The quote email could not be sent; no sent or quoted state was saved",
      });
      return;
    }
