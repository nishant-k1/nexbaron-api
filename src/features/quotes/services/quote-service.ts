...
export function nextQuoteNumber(
  division: "digital" | "print",
  InvoiceCounter: Model<any>,
): Promise<string> {
  const year = new Date().getFullYear();
  const prefix = division === "digital" ? "NXB-D" : "NXB-P";
  return nextSequence(InvoiceCounter, `quote-${division}-${year}`).then(
    (seq) => `${prefix}-${year}-${String(seq).padStart(5, "0")}`,
  );
}

export function whatsAppDelivery(quote: IQuote): {
  available: boolean;
  link?: string;
} {
  ...
  const message = quote.response?.message
    ...
    : encodeURIComponent(
        `Hi ${quote.customer.name}, your ${BRANDS[quote.division].name} quote is ready.`,
      );
  return {
    available: true,
    link: `https://wa.me/${phone.replace(/[^\d]/g, "")}?text=${message}`,
  };
}

export function selectionSummary(quote: IQuote): string {
  if (quote.division === "digital") {
    const planIds = Array.isArray(quote.selection?.planIds)
      ? (quote.selection.planIds as string[])
      : [];
    const plans = planIds
  ...
  return (
    [product && getProductLabel(product), quantity && `${quantity} units`]
      .filter(Boolean)
      .join("\n") || "—"
  );
}
...
export function quoteHtml(quote: IQuote): string {
  ...
  const monthly = resp?.monthlyPrice
    ? resp.monthlyPrice.toLocaleString("en-IN")
    : null;
...
export function renderQuotePdf(quote: IQuote): Promise<Buffer> {
  ...
  return new Promise((resolve, reject) => {
    ...
    doc
      .fontSize(9)
      .fillColor("#e2e8f0")
      .text(`Quote: ${quote.quoteNumber}`, 114, 118, { align: "right" });
    doc
      .fontSize(7)
      .fillColor("#94a3b8")
      .text(`Valid ${resp?.validityDays ?? 7} days`, 114, 130, {
        align: "right",
      });
    ...
    doc
      ...
      .fontSize(9)
      .fillColor("#64748b")
      .text("Requested", col1, y0 + 9);
    doc
      .font("Helvetica")
      .fontSize(10)
      .fillColor("#0f172a")
      .text(selectionSummary(quote), 160, y0 + 8);
    ...
    if (quote.customer.company) {
      ...
      doc
        ...
        .fontSize(9)
        .fillColor("#64748b")
        .text("Company", col1, y + 9);
      doc
        .font("Helvetica")
        .fontSize(10)
        .fillColor("#0f172a")
        .text(quote.customer.company, 160, y + 8);
      y += rowH;
    }
    ...
    if (resp?.monthlyPrice) {
      doc
        ...
        .fillColor("#64748b")
        .text(
          `+ ₹${resp.monthlyPrice.toLocaleString("en-IN")}/month`,
          160 +
            doc.widthOfString(
              `₹${(resp?.price ?? 0).toLocaleString("en-IN")}`,
            ) +
            8,
          y + 10,
        );
    ...
    doc
      .fontSize(8)
      .fillColor("#94a3b8")
      .text(brand.name, 56, doc.y, { width: pageWidth / 2, align: "left" });
    doc.text("nexbaron.com", 56, doc.y - 10, {
      width: pageWidth,
      align: "right",
    });
